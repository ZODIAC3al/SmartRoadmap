...
const STATUS_CFG: Record<SalaryDataStatus, {
  ...
  NO_DATA: {
    badge: "— No Data", badgeCls: "bg-base-content/10 text-base-content/70 dark:text-stone-300 font-medium border-base-content/20",
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 text-center">
        <div>
          <p className="text-[9px] font-bold uppercase tracking-wider text-base-content/70 dark:text-stone-400 font-medium font-mono">Min</p>
          <p className="text-sm font-black text-base-content tabular-nums mt-0.5">{fmt(min, currency)}</p>
          <p className="text-[9px] text-base-content/70 dark:text-stone-400 font-medium font-mono">{periodLabel}</p>
        </div>
        <div>
        ...
        <div>
          <p className="text-[9px] font-bold uppercase tracking-wider text-base-content/70 dark:text-stone-400 font-medium font-mono">Max</p>
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center gap-3">
        ...
        <span className="text-[10px] font-bold text-base-content/70 dark:text-stone-300 font-medium font-mono w-16 text-right">{score}% · {label}</span>
      </div>
      {jobsAnalyzed > 0 && (
        <p className="text-[10px] text-base-content/70 dark:text-stone-400 font-medium font-mono">
      ...
      {trends.map((t) => {
        ...
        return (
          <div key={t.year} className="flex flex-col items-center flex-1 gap-1">
            <span className="text-[9px] text-base-content/70 dark:text-stone-300 font-medium font-mono leading-none">{shortLabel(t.averageSalary)}</span>
            ...
            <span className={`text-[9px] font-bold font-mono ${isCurrent ? "text-[#7c3aed]" : "text-base-content/70 dark:text-stone-300 font-medium"}`}>{t.year}</span>
          </div>
        );
      })}
    </div>
...
function CountrySelector({ selected, onChange, disabled }: {
  ...
  return (
    <div className="bg-base-200 border border-base-300 rounded-2xl px-5 py-4 shadow-sm">
      <p className="text-[10px] font-bold uppercase tracking-wider text-base-content/70 dark:text-stone-300 font-medium font-mono mb-3">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold
                ...
                ${active ? "bg-[#7c3aed] text-white border-[#7c3aed] shadow-sm"
                         : "bg-base-100 text-base-content/70 dark:text-stone-300 font-medium border-base-300 hover:border-[#7c3aed]/50 hover:text-[#7c3aed]"}
...
function DataStatusCard({ status, sourceLabel, onRefresh }: {
  ...
  return (
    <div className={`rounded-2xl px-5 py-4 border shadow-sm ${isError ? "bg-error/5 border-error/20" : "bg-warning/5 border-warning/20"}`}>
      ...
      <p className="text-xs text-base-content/70 dark:text-stone-300 font-medium leading-relaxed">{cfg.body}</p>
      <div className="flex items-center justify-between mt-3">
        <span className="text-[10px] text-base-content/70 dark:text-stone-400 font-medium font-mono">{sourceLabel}</span>
        <button onClick={onRefresh}
          className="btn btn-xs btn-ghost border border-base-300 rounded-xl text-[10px] font-bold gap-1.5 hover:text-[#7c3aed] transition-all duration-300 ease-in-out">
  ...
  return (
    <div className="flex flex-col items-center justify-center py-14 gap-4 text-center">
      ...
      <div>
        ...
        <p className="text-xs text-base-content/70 dark:text-stone-300 font-medium mt-1 max-w-xs">
          Select a country above, then make sure your career profile has a current role and skills saved.
        </p>
      </div>
      <button onClick={onRefresh}
        className="btn btn-sm bg-[#7c3aed] hover:bg-[#6d28d9] border-none text-white rounded-xl text-xs font-bold px-6 transition-all duration-300 ease-in-out">
        Load Insights
      </button>
    </div>
  );
          ...
          <>
            ...
            <div className="bg-base-200 border border-base-300 rounded-2xl px-5 py-4 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex flex-col gap-1">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-base-content/70 dark:text-stone-400 font-medium font-mono">
                    Salary comparison for
                  </p>
                  <p className="text-base font-black text-base-content leading-tight">{jobTitle}</p>
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    {countryOpt && (
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold text-base-content/70 dark:text-stone-300 font-medium">
                        <span className="text-sm">{countryOpt.flag}</span>{countryOpt.label}
                      </span>
                    )}
                    <span className="text-base-content/70 dark:text-stone-400 font-medium text-[10px]">·</span>
                    <span className="text-[10px] font-bold font-mono text-base-content/70 dark:text-stone-300 font-medium">{displayCurrency}</span>
                    <span className="text-base-content/70 dark:text-stone-400 font-medium text-[10px]">·</span>
                    <StatusBadge status={dataStatus} />
                    {cachedLabel && (
                      <span className="text-[10px] text-base-content/70 dark:text-stone-400 font-medium font-mono">· {cachedLabel}</span>
                    )}
                  </div>
                </div>
                <button onClick={onRefresh}
                  className="btn btn-xs btn-ghost border border-base-300 rounded-xl text-[10px] font-bold
                    text-base-content/70 dark:text-stone-300 font-medium hover:text-[#7c3aed] hover:border-[#7c3aed]/40 gap-1.5 self-start sm:self-center transition-all duration-300 ease-in-out">
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round"
                      d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
            ...
            {showSalary && (
              <div className="bg-base-200 border border-base-300 rounded-2xl px-5 py-5 shadow-sm space-y-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-base-content/70 dark:text-stone-300 font-medium font-mono">
            ...
            {showSalary && salaryRecommendation && (
              <div className="bg-[#7c3aed]/5 border border-[#7c3aed]/20 rounded-2xl px-5 py-4 shadow-sm">
                ...
                <p className="text-xs text-base-content dark:text-stone-200 font-medium leading-relaxed">{salaryRecommendation}</p>
              </div>
            )}
                  ...
                  <div className="bg-base-200 border border-base-300 rounded-2xl p-5 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {trendingSkills.map((s) => (
                        <span key={s} className="px-2.5 py-1 bg-[#7c3aed]/10 text-[#7c3aed] border border-[#7c3aed]/20 rounded-2xl text-[11px] font-bold">{s}</span>
                      ))}
                    </div>
                  </div>
                )}
            ...
            {skillGapAnalysis && (skillGapAnalysis.missingSkills.length > 0 || skillGapAnalysis.recommendations.length > 0) && (
              <div className="bg-base-200 border border-base-300 rounded-2xl p-5 shadow-sm">
                <p className="text-[10px] font-bold uppercase tracking-wider text-base-content/70 dark:text-stone-300 font-medium font-mono mb-4">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skillGapAnalysis.missingSkills.map((s) => (
                          <span key={s} className="px-2.5 py-1 bg-error/10 text-error border border-error/20 rounded-2xl text-[11px] font-semibold">{s}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {skillGapAnalysis.recommendations.map((rec, i) => (
                    <div key={i} className="flex items-start gap-2.5 p-3 bg-[#7c3aed]/5 border border-[#7c3aed]/15 rounded-xl">
                      ...
                      <p className="text-xs text-base-content dark:text-stone-200 font-medium leading-relaxed">{rec}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* ── Footer ───────────────────────────────────────────────── */}
            <p className="text-[10px] text-base-content/70 dark:text-stone-400 font-medium text-center font-mono pb-1">
              {sourceLabel}
