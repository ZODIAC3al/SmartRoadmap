...
export const ModernPortfolio: React.FC<Props> = ({ portfolio }) => {
  ...
  return (
    <div className="w-full min-h-full bg-slate-900 text-slate-100 font-sans select-text p-6 sm:p-12 space-y-12">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-3xl p-8 sm:p-12 text-white shadow-xl space-y-4">
        ...
        <div className="flex flex-wrap gap-4 pt-4 text-xs font-bold">
          {socialLinks?.github && (
            <a href={socialLinks.github} target="_blank" rel="noreferrer" className="btn btn-sm bg-white text-slate-900 hover:bg-slate-100 border-none rounded-xl font-bold">
              GitHub Profile
            </a>
          )}
          {socialLinks?.linkedin && (
            <a href={socialLinks.linkedin} target="_blank" rel="noreferrer" className="btn btn-sm bg-blue-500 hover:bg-blue-600 text-white border-none rounded-xl font-bold">
              LinkedIn
            </a>
          )}
        </div>
      ...
      {projects && projects.length > 0 && (
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.map((proj, i) => (
              <div key={i} className="bg-slate-800/80 border border-slate-700/80 rounded-3xl p-6 space-y-4 shadow-lg hover:border-indigo-500 transition-colors">
                ...
                {proj.technologies && proj.technologies.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {proj.technologies.map((t, idx) => (
                      <span key={idx} className="text-[10px] bg-slate-900 text-indigo-300 font-mono px-2 py-0.5 rounded-md">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex gap-3 text-xs font-bold pt-2">
                  {proj.githubUrl && (
                    <a href={proj.githubUrl} target="_blank" rel="noreferrer" className="text-indigo-400 hover:underline">
                      GitHub →
                    </a>
                  )}
                  {proj.demoLink && (
                    <a href={proj.demoLink} target="_blank" rel="noreferrer" className="text-purple-400 hover:underline">
                      Live Preview ↗
                    </a>
                  )}
                </div>
