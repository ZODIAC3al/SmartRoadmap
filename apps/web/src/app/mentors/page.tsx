  ...
  return (
    <div className={`min-h-screen text-base-content pb-16 px-4 sm:px-6 lg:px-8 font-sans ${isRtl ? "text-right" : "text-left"}`}>
      <div className="max-w-6xl mx-auto space-y-8 pt-4">
          ...
          <div className="bg-base-200/80 backdrop-blur-md border border-base-300 rounded-3xl p-6 space-y-4 shadow-xl">
            <div className="flex items-center justify-between border-b border-base-300 pb-3">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-xl bg-amber-500/10 text-amber-500 text-[10px] font-black uppercase tracking-wider font-mono border border-amber-500/20 flex items-center gap-1">
              ...
              {recommendedMentors.slice(0, 2).map((rec) => (
                <div key={rec._id} className="bg-base-100 border border-base-300 p-4 rounded-2xl flex gap-3 relative overflow-hidden hover:border-[#8E1616]/40 transition-all shadow-md">
                  ...
                  <div className="space-y-1.5 flex-1 min-w-0">
                    ...
                    <span className="text-[9px] text-base-content/70 dark:text-stone-300 font-medium font-bold block uppercase leading-none font-mono">
                    ...
                    <button
                      ...
                      className="btn btn-xs bg-[#8E1616] hover:bg-[#8E1616] text-white border-none rounded-xl px-3.5 mt-1 font-bold gap-1 transition-all duration-300 ease-in-out"
                    >
        ...
        {/* Sessions Manager */}
        <div className="bg-base-200/80 backdrop-blur-md border border-base-300 rounded-3xl p-6 space-y-4 shadow-xl">
          <h3 className="font-extrabold text-xs uppercase tracking-wider font-mono text-base-content/70 dark:text-stone-300 font-medium flex items-center gap-1.5">
          ...
          <div className="overflow-x-auto">
            {sessions.length === 0 ? (
              <p className="text-xs text-base-content/70 dark:text-stone-400 font-medium text-center py-8 italic font-semibold">
                {isRtl ? "لا توجد جلسات مجدولة حالياً." : "No scheduled sessions found."}
              </p>
            ) : (
              <table className="table w-full text-xs">
                <thead>
                  <tr className="border-b border-base-300 uppercase font-mono text-[9px] text-base-content/70 dark:text-stone-300 font-medium">
                  ...
                  {sessions.map((sess) => {
                    ...
                    return (
                      <tr key={sess._id} className="border-b border-base-300/60 hover:bg-base-100/40 transition-colors">
                        <td>
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              <span className="text-[9px] text-base-content/70 dark:text-stone-400 font-medium block font-mono">
                        ...
                        <td className="max-w-[150px] truncate text-base-content/70 dark:text-stone-300 font-medium">{sess.notes || "-"}</td>
                        <td>
                          <span className={`badge border-none font-extrabold text-[9px] uppercase px-2.5 py-1 rounded-2xl font-mono ${
                            ...
                            sess.status === "completed" ? "bg-primary text-primary-content/15 text-primary border border-primary/30" : "bg-base-300 text-base-content/70 dark:text-stone-300 font-medium"
                          }`}>
                            {sess.status}
                          </span>
                          {sess.feedback && (
                            <p className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium italic mt-1 block">
                              📝 Feed: {sess.feedback}
                            </p>
                          )}
                        </td>
                          ...
                          <div className="flex gap-1.5">
                            {!isLearnerSession && sess.status === "pending" && (
                              <>
                                <button
                                  ...
                                  className="btn btn-xs bg-[#8E1616] hover:bg-[#8E1616]/10 text-white border-none rounded-2xl p-1 transition-all duration-300 ease-in-out"
                                  title="Accept"
                                >
                                ...
                                <button
                                  ...
                                  className="btn btn-xs bg-red-500 hover:bg-red-600 text-white border-none rounded-2xl p-1 transition-all duration-300 ease-in-out"
                                  title="Reject"
                                >
                            ...
                            {!isLearnerSession && sess.status === "accepted" && (
                              <button
                                ...
                                className="btn btn-xs bg-primary text-primary-content hover:bg-blue-700 text-white border-none rounded-2xl font-bold transition-all duration-300 ease-in-out"
                              >
                            ...
                            {isLearnerSession && sess.status === "completed" && (
                              <button
                                ...
                                className="btn btn-xs bg-amber-500 hover:bg-amber-600 text-white border-none rounded-2xl font-bold flex items-center gap-1 transition-all duration-300 ease-in-out"
                              >
                            ...
                            {sess.status === "pending" && (
                              <button
                                ...
                                className="btn btn-xs btn-ghost text-red-500 hover:bg-red-500/10 font-bold transition-all duration-300 ease-in-out"
                              >
                                {isRtl ? "إلغاء" : "Cancel"}
                              </button>
                            )}
        ...
        <div className="space-y-4">
          <form onSubmit={handleSearch} className="bg-base-200/80 backdrop-blur-md border border-base-300 p-2.5 rounded-2xl flex gap-2 max-w-md shadow-lg">
            ...
            />
            <button type="submit" className="btn bg-[#8E1616] hover:bg-[#8E1616] text-white border-none rounded-xl text-xs px-4 gap-1.5 transition-all duration-300 ease-in-out">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {mentors.length === 0 ? (
              <p className="text-xs text-base-content/70 dark:text-stone-400 font-medium text-center py-12 col-span-full font-bold">
                ...
                <div key={mentor._id} className="bg-base-200/80 backdrop-blur-md border border-base-300 p-5 rounded-3xl space-y-4 shadow-xl hover:border-[#8E1616]/20/30 transition-all flex flex-col justify-between">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5 text-xs">
                      <div className="flex justify-between font-mono text-[9px] text-base-content/70 dark:text-stone-300 font-medium">
                        <span>INDUSTRY:</span>
                      ...
                      <p className="text-[11px] text-base-content/70 dark:text-stone-300 font-medium leading-normal line-clamp-3 pt-1">
                        {mentor.bio}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {mentor.expertise.map((exp) => (
                        <span key={exp} className="text-[9px] bg-base-100 text-base-content/70 dark:text-stone-300 font-medium font-mono px-2.5 py-0.5 rounded-2xl border border-base-300 font-bold">
                          {exp}
                        </span>
                      ))}
                    </div>
                  ...
                  {!userIsMentor && (
                    <button
                      ...
                      className="btn btn-sm bg-[#8E1616] hover:bg-[#8E1616] text-white border-none rounded-2xl font-bold w-full gap-2 shadow-md mt-2 transition-all duration-300 ease-in-out"
                    >
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-base-200 border border-base-300 w-full max-w-sm p-6 rounded-3xl shadow-2xl space-y-4">
              ...
              <div className="bg-[#8E1616]/10 border border-[#8E1616]/20/20 p-3 rounded-2xl space-y-1.5">
                ...
                <div className="space-y-0.5 text-[10px] text-base-content/70 dark:text-stone-300 font-medium">
              ...
              <form onSubmit={handleBookSession} className="space-y-4 text-xs font-semibold">
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "التاريخ" : "Date"}</label>
                    <input
                      type="date"
                      required
                      value={bookingDate}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "الوقت" : "Time"}</label>
                    <input
                      type="time"
                      required
                      value={bookingTime}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "ملاحظات وأهداف الجلسة" : "Session goals"}</label>
                  <textarea
                    rows={2}
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-base-200 border border-base-300 w-full max-w-md p-6 rounded-3xl shadow-2xl space-y-4 max-h-[85vh] overflow-y-auto">
              ...
              <form onSubmit={handleSetupProfile} className="space-y-4 text-xs font-semibold">
                <div className="space-y-1">
                  <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "مجالات الخبرة (مفصولة بفاصلة)" : "Expertise tags (comma separated)"}</label>
                  <input
                    type="text"
                    required
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "سنوات الخبرة" : "Years of Experience"}</label>
                    <input
                      type="number"
                      required
                      value={expYearsInput}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "مجال الصناعة" : "Industry Segment"}</label>
                    <input
                      type="text"
                      required
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "النبذة التعريفية" : "Professional Bio"}</label>
                  <textarea
                    required
                    rows={3}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "الشهادات المهنية (مفصولة بفاصلة)" : "Certifications"}</label>
                  <input
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] text-base-content/70 dark:text-stone-300 font-medium uppercase block">{isRtl ? "أوقات التوافر الأسبوعية" : "Weekly Availability Slots"}</label>
                  {slotsInput.map((slot, index) => (
                    <div key={index} className="flex gap-1.5 items-center">
                      ...
                      />
                      <span className="text-[10px] text-base-content/70 dark:text-stone-400 font-medium font-bold font-mono">-</span>
                      <input
                        type="time"
                        value={slot.endTime}
                        onChange={(e) => {
                      ...
                      <button
                        ...
                        className="btn btn-circle btn-xs btn-ghost text-red-500 hover:bg-red-500/10 transition-all duration-300 ease-in-out"
                      >
                        <X className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="text-xs text-primary font-bold hover:underline flex items-center gap-1 pt-1 transition-all duration-300 ease-in-out"
                  >
