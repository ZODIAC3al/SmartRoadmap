...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'account' | 'security' | 'notifications' | 'interface' | 'additional'>('account');
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        setName(u.name || '');
        setEmail(u.email || '');
        setUsername(u.username || (u.name ? u.name.toLowerCase().replace(/\s+/g, '') : ''));
        setPhone(u.phone || '');
        setBio(u.bio || '');
        setRole(u.role || 'learner');
        setAvatarChar((u.name || 'Ali Maher').split(' ').map((n: string) => n[0]).join(''));
      } catch (e) {}
    }
    setLoading(false);
  }, []);
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(locale === 'en' ? 'Profile details updated successfully!' : 'تم تحديث بيانات الملف الشخصي بنجاح!');
    } catch (err: any) {
      toast.error(locale === 'en' ? `Update failed: ${err.message}` : `فشل التحديث: ${err.message}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const sidebarLinks = [
    { label: t('profile.sidebar.home'), href: '/dashboard', icon: <HomeIcon /> },
    ...
    { label: t('profile.sidebar.settings'), href: '/profile', icon: <SettingsIcon />, active: true },
  ];
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <header className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 border-b border-base-300">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-extrabold text-sm tracking-tight text-base-content block">deep creationz</span>
              <span className="text-[9px] text-base-content/40 uppercase font-bold font-mono tracking-widest leading-none">smartroadmap</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              {user?.avatarUrl ? (
                <img src={user.avatarUrl} alt="Avatar" className="w-8 h-8 rounded-full object-cover border border-base-300 shadow-sm" />
              ) : (
              ...
              <div className="hidden md:block text-left">
                <span className="text-[10px] text-base-content/50 block font-semibold leading-none">{t('profile.header.welcome')}</span>
                <span className="text-xs font-bold text-base-content mt-1 block leading-none">{name || 'Christine Brown'}</span>
              </div>
          ...
          <main className="col-span-1 md:col-span-9 flex flex-col space-y-6">
            ...
            <div className="flex-grow">
              ...
              {activeTab === 'account' && (
                <form onSubmit={handleSaveProfile} className="bg-base-200 border border-base-300 rounded-2xl p-6 md:p-8 space-y-6 shadow-sm">
                  ...
                  <div className="space-y-3 text-start">
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                      ...
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() => document.getElementById('accountAvatarFileInput')?.click()}
                  ...
                  <div className="pt-2 flex justify-end">
                    ...
                    >
                      {isSaving && <span className="loading loading-spinner loading-xs mr-2" />}
              ...
              {activeTab === 'security' && (
                <div className="bg-base-200 border border-base-300 rounded-2xl p-6 md:p-8 space-y-6 shadow-sm text-start">
                  <h3 className="font-extrabold text-sm text-base-content border-b border-base-300 pb-3 uppercase tracking-wider font-mono">
                    {locale === 'en' ? 'Update Login Password' : 'تغيير كلمة المرور'}
                  </h3>
                  
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="form-control">
                      <label className="label text-[10px] font-bold uppercase text-base-content/40 font-mono">Current Password</label>
                      <input type="password" placeholder="••••••••" className="input input-bordered w-full rounded-xl bg-base-100 text-xs h-10 border-base-300" />
                    </div>
                    <div className="form-control">
                      <label className="label text-[10px] font-bold uppercase text-base-content/40 font-mono">New Password</label>
                      <input type="password" className="input input-bordered w-full rounded-xl bg-base-100 text-xs h-10 border-base-300" />
                    </div>
                    <div className="form-control">
                      <label className="label text-[10px] font-bold uppercase text-base-content/40 font-mono">Confirm Password</label>
                      <input type="password" className="input input-bordered w-full rounded-xl bg-base-100 text-xs h-10 border-base-300" />
                    </div>
                  </div>

                  <div className="flex justify-end pt-2">
                    <button onClick={() => toast.success('Password update simulated.')} className="btn bg-[#7c3aed] hover:bg-[#6d28d9] border-none text-white text-xs rounded-xl h-10 min-h-0 px-6 font-bold uppercase">
                      Save Password
                    </button>
                  </div>
                </div>
              ...
              {activeTab === 'notifications' && (
                <div className="bg-base-200 border border-base-300 rounded-2xl p-6 md:p-8 space-y-6 shadow-sm text-start">
                  <h3 className="font-extrabold text-sm text-base-content border-b border-base-300 pb-3 uppercase tracking-wider font-mono">
                    {locale === 'en' ? 'Notification Subscriptions' : 'اشتراكات الإشعارات والتنبيهات'}
                  </h3>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-3 bg-base-100 border border-base-300 rounded-xl">
                      <div>
                        ...
                        <p className="text-[10px] text-base-content/55 mt-0.5">Receive immediate notification when your CV matches new job openings.</p>
                      </div>
                      <input 
                        type="checkbox" 
                    ...
                    <div className="flex items-center justify-between p-3 bg-base-100 border border-base-300 rounded-xl">
                      <div>
                        ...
                        <p className="text-[10px] text-base-content/55 mt-0.5">Weekly notifications for unlocked roadmap milestone challenges.</p>
                      </div>
                      <input 
                        type="checkbox" 
                        className="toggle toggle-primary toggle-sm" 
                    ...
                    <div className="flex items-center justify-between p-3 bg-base-100 border border-base-300 rounded-xl">
                      <div>
                        <p className="font-bold text-xs">Newsletter & Updates</p>
                        <p className="text-[10px] text-base-content/55 mt-0.5">Receive digests of candidate verified metrics and hiring trends.</p>
                      </div>
                      <input 
                        type="checkbox" 
                        className="toggle toggle-primary toggle-sm" 
                  ...
                  <div className="flex justify-end pt-2">
                    <button onClick={() => toast.success('Notification selections saved.')} className="btn bg-[#7c3aed] hover:bg-[#6d28d9] border-none text-white text-xs rounded-xl h-10 min-h-0 px-6 font-bold uppercase">
                      Save Settings
                    </button>
                  </div>
                </div>
              ...
              {activeTab === 'interface' && (
                <div className="bg-base-200 border border-base-300 rounded-2xl p-6 md:p-8 space-y-6 shadow-sm text-start">
                  <h3 className="font-extrabold text-sm text-base-content border-b border-base-300 pb-3 uppercase tracking-wider font-mono">
                    {locale === 'en' ? 'Interface Configurations' : 'إعدادات واجهة المستخدم'}
                  </h3>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="form-control space-y-2">
                      <label className="text-[10px] font-bold uppercase text-base-content/40 block font-mono">
                        {locale === 'en' ? 'Visual Theme Mode' : 'وضع المظهر المرئي'}
                      </label>
                      <div className="flex gap-2">
                        <button
                          type="button"
                    ...
                    <div className="form-control space-y-2">
                      <label className="text-[10px] font-bold uppercase text-base-content/40 block font-mono">
                        {locale === 'en' ? 'Application Language' : 'لغة التطبيق الافتراضية'}
                      </label>
                      <div className="flex gap-2">
                        <button
                          type="button"
              ...
              {activeTab === 'additional' && (
                <div className="bg-base-200 border border-base-300 rounded-2xl p-6 md:p-8 space-y-6 shadow-sm text-start">
                  ...
                  <div className="p-4 bg-error/5 border border-error/20 rounded-xl space-y-4">
                    <div>
                      <h4 className="font-bold text-xs text-error">Delete Workspace Account</h4>
                      <p className="text-[10px] text-base-content/60 mt-1">
                        Once confirmed, all completed learning paths, roadmap quiz records, and matching indices will be permanently scrubbed from MongoDB.
                      </p>
                    </div>
                    <button 
