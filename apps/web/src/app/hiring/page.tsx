...
export default function HiringPage() {
    ...
    async function fetchMatches() {
      try {
        ...
        const enriched = data.map((job: any, index: number) => ({
          ...
          budget: job.salaryMin ? `$${job.salaryMin}${job.salaryMax ? ` - $${job.salaryMax}` : ''}` : index === 0 ? '$125,000 - $140,000' : index === 1 ? '$95,000 - $110,000' : '$115,000 - $130,000',
          overallSpent: index === 0 ? '$50k' : 'N/A',
          clientRating: index === 0 ? '5★' : 'N/A',
          aiExplanation: index === 0 
        ...
        const fallbackJobs: ScoredJob[] = [
          {
            ...
            requiredSkills: ['React Framework Architecture', 'TypeScript Strict Mode Interfaces', 'Tailwind Design System Tokens', 'CI/CD Pipelines'],
            remote: true,
            description: 'We are looking for an experienced frontend designer to expand our payment interface component libraries, set visual token boundaries, and write tests.',
            matchScore: 97,
            ...
            overallSpent: '$1.2M',
            clientRating: '5.0★',
            aiExplanation: 'Exceptional match! Your verified React (92%) and TypeScript (95%) scores exceed the candidate baseline. Closing CI/CD gaps will achieve 100% compatibility.'
          },
          {
            _id: 'mock-2',
            ...
            location: 'Remote',
            country: 'United States',
            requiredSkills: ['React Framework Architecture', 'Next.js App Router Prefetching', 'Docker Basics'],
            remote: true,
            description: 'Seeking a developer focused on rendering pipeline optimization and edge-computing templates. Docker configuration experience is nice to have.',
            matchScore: 92,
            ...
            overallSpent: '$800k',
            clientRating: '4.9★',
            aiExplanation: 'Strong compatibility. The team requires React prototyping skills which you possess. The docker gap is easily covered in module 4.'
          },
          {
            _id: 'mock-3',
            ...
            location: 'Hybrid',
            country: 'France',
            requiredSkills: ['React Framework Architecture', 'TypeScript Strict Mode Interfaces'],
            remote: false,
            description: 'Join our client interface team to build fast keyboard-driven features. Focus on design tokens alignment and clean, type-safe API consumption.',
            matchScore: 89,
            skillsGap: [],
            ...
            overallSpent: '$300k',
            clientRating: '4.8★',
            aiExplanation: 'Excellent alignment. You have complete 100% skill compatibility with all requirements posted for this role. Zero skill gaps detected!'
          }
        ];
  ...
  const handleApply = (company: string, jobTitle: string) => {
    toast.success(`Application sent successfully to ${jobTitle} at ${company}!`);
    ...
    <div className="min-h-screen bg-base-100 text-base-content pb-8 px-4 sm:px-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="text-start border-b border-base-300 pb-6">
          <span className="text-[10px] text-base-content/40 font-bold uppercase tracking-wider font-mono">vector match ranking</span>
          <h1 className="text-3xl font-black tracking-tight text-base-content mt-1">Hiring Match Pipeline</h1>
          <p className="text-xs text-base-content/50 mt-1">Jobs recommended based on semantic matching with your Skill Passport credentials.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <aside className="lg:col-span-3 bg-base-200 border border-base-300 rounded-2xl p-5 text-start space-y-6">
            <div className="flex justify-between items-center border-b border-base-300 pb-3">
              <span className="text-xs font-bold uppercase tracking-wider font-mono text-base-content/40">Search Filters</span>
              <button 
                onClick={() => {
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-base-content/40 uppercase tracking-wider block font-mono">Job Title / Keyword</label>
              <input
                type="text"
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-[10px] font-bold text-base-content/40 uppercase font-mono">
                ...
                <span className="text-[#059669] font-black">{filterMatchScore}%</span>
              </div>
              <input
                type="range"
                min="50"
            ...
            <div className="border border-green-200 bg-green-50/50 p-4 rounded-xl space-y-2 text-xs">
              <span className="font-bold text-[#059669] block">Vetted Candidate Profile</span>
          ...
          <section className="lg:col-span-5 space-y-4 text-start">
            <span className="text-xs font-bold uppercase tracking-wider text-base-content/40 font-mono block mb-2">Available matches ({filteredJobs.length})</span>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <h3 className="font-extrabold text-xs text-base-content leading-tight pr-4">{job.title}</h3>
                        <p className="text-[10px] text-base-content/40 font-semibold mt-1">{job.company} • {job.location}</p>
                      </div>
                    ...
                    <div className="flex justify-between items-center text-[10px] text-base-content/50 border-t border-base-300 pt-3">
                      <span>Salary: <span className="font-bold text-base-content">{job.budget}</span></span>
          ...
          <aside className="lg:col-span-4 lg:sticky lg:top-24 text-start">
            {selectedJob ? (
              <div className="bg-base-200 border border-base-300 rounded-2xl p-6 shadow-sm space-y-6">
                <div>
                  <span className="text-[10px] text-base-content/40 font-mono font-bold uppercase tracking-wider">AI MATCH ANALYSIS</span>
                  <h3 className="text-md font-black text-base-content mt-1">{selectedJob.title}</h3>
                  <p className="text-[10px] text-base-content/40 font-semibold mt-0.5">{selectedJob.company} • {selectedJob.location}</p>
                </div>

                {/* AI Explanation block */}
                <div className="bg-base-100 border border-base-300 rounded-xl p-4 space-y-2">
                  <span className="text-[10px] text-base-content/40 font-bold uppercase tracking-wider font-mono block">Why You Match</span>
                ...
                <div className="space-y-3.5">
                  <span className="text-[10px] text-base-content/40 font-bold uppercase tracking-wider font-mono block">Required Skills & Gaps</span>
                  ...
                  {selectedJob.skillsGap.length > 0 && (
                    <button
                      onClick={() => handleAddSkills(selectedJob._id, selectedJob.skillsGap)}
                    ...
                    >
                      {addingSkills.includes(selectedJob._id) ? 'Injecting...' : '⚡ Inject missing skills into Roadmap'}
                    </button>
                  )}
                </div>
                ...
                {/* Apply Trigger */}
                <button
                  onClick={() => handleApply(selectedJob.company, selectedJob.title)}
      ...
      {successApplyJob && (
        <div className="modal modal-open">
          <div className="modal-box rounded-2xl bg-base-200 border border-base-300 text-center space-y-4">
            ...
            <h3 className="font-extrabold text-lg text-base-content">Application Submitted!</h3>
            <p className="text-xs text-base-content/50 leading-relaxed">
              Your profile, parsed CV details, and verified quiz scores have been transmitted successfully. Recruiter teams will review your credentials directly.
            </p>
            <div className="modal-action justify-center">
              <button onClick={() => setSuccessApplyJob(null)} className="btn bg-[#10B981] hover:bg-[#059669] border-none text-white btn-sm rounded-lg px-8">
                Confirm
              </button>
            </div>
          </div>
