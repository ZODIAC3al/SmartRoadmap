...
export function useCompanyDashboard() {
  ...
  const [activeCvPreview, setActiveCvPreview] = useState<Candidate | null>(
    null,
  );
  ...
  const [interviewNote, setInterviewNote] = useState(
    "Hi, I reviewed your Skill Passport and was highly impressed by your NestJS & React scores. I would love to schedule a direct interview.",
  );
  ...
  const fallbackCandidates: Candidate[] = [
    {
      ...
      capstoneProject: {
        ...
        verified: true,
        auditLog:
          "AI code review passed on June 18, 2026. Codebase is clean, matches enterprise standards, and has 92% unit test coverage.",
      },
    },
    {
      userId: "cand-ali",
      ...
      capstoneProject: {
        ...
        verified: true,
        auditLog:
          "Design spec audit passed. Layout is fully responsive, achieves 100 on Lighthouse accessibility parameters, and contains robust dark-mode variables.",
      },
    },
    {
      userId: "cand-marina",
      ...
      capstoneProject: {
        ...
        verified: true,
        auditLog:
          "Telemetry verify passed. Successfully sustained 10,000 mock concurrent WebSockets connections with less than 20ms latency responses.",
      },
    },
    {
      userId: "cand-nada",
    ...
    try {
      ...
      const enriched: Candidate[] = data.map((c: any, index: number) => {
        const fallback =
          ...
          ) || fallbackCandidates[index % fallbackCandidates.length];
        return {
          ...c,
      ...
      console.warn(
        "Failed fetching candidates from API, falling back to local simulation data.",
      );
  ...
  const handleSendInterviewInvite = (e: React.FormEvent) => {
    ...
    toast.success(
      `Interview invitation transmitted to ${contactCandidate.name}!`,
    );
  ...
  const handleAddJobPosting = (e: React.FormEvent) => {
    ...
    setNewJobSkills("");
    toast.success(
      `New Job Profile "${newJob.title}" added to matching vectors!`,
    );
  };
  ...
  const getDynamicMatchScore = (candidate: Candidate) => {
    ...
    job.skills.forEach((reqSkill) => {
      const matchFound = lowerSkills.some(
        (v) =>
          v.includes(reqSkill.toLowerCase()) ||
          reqSkill.toLowerCase().includes(v),
      );
