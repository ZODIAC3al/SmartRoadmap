    ...
    <header className="sticky top-4 z-50 px-4 w-full">
      <div className="max-w-5xl mx-auto rounded-full bg-base-200/90 backdrop-blur-md text-base-content border border-base-300 shadow-lg px-3 sm:px-6 h-14 flex items-center justify-between gap-2 sm:gap-4 transition-all duration-200">
        ...
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <img src="/logo.svg" alt="SmartRoadmap Logo" className="w-8 h-8 sm:w-9 sm:h-9 hover:scale-105 transition-transform duration-200" />
          <span className="hidden sm:inline font-black tracking-tight text-sm text-base-content">{t('nav.logo')}</span>
        </Link>
          ...
          {navLinks.filter((l) => l.show).map((link) => (
            <Link
              ...
              className={`transition-colors hover:text-[#10B981] ${
                isLinkActive(link.href) ? 'text-[#10B981] font-black' : 'text-base-content/65'
              }`}
            >
              {link.label}
            </Link>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            ...
            title={locale === 'en' ? `Download Standalone App (${os.toUpperCase()})` : `تحميل التطبيق المستقل (${os.toUpperCase()})`}
          >
          ...
          {user && (
            <div className="hidden xs:block dropdown dropdown-end">
              <label tabIndex={0} className="btn btn-ghost btn-circle btn-xs text-base-content/85 hover:bg-base-300 flex items-center justify-center relative cursor-pointer">
                <BellIcon />
              ...
              <div tabIndex={0} className="dropdown-content mt-3 z-[150] p-4 shadow-xl bg-base-200 text-base-content border border-base-300 rounded-2xl w-72 sm:w-96 space-y-3 font-sans">
                ...
                <div className="max-h-60 overflow-y-auto space-y-2">
                  {notifications.length === 0 ? (
                    <p className="text-[10px] text-base-content/40 text-center py-6">
                      {locale === 'en' ? 'No notifications yet.' : 'لا توجد تنبيهات حالياً.'}
                    </p>
                  ) : (
                ...
                <div className="border-t border-base-300 pt-2 text-center">
                  ...
                  >
                    {locale === 'en' ? 'See all notifications' : 'عرض كافة التنبيهات'}
                  </Link>
                </div>
              </div>
            </div>
          ...
          {user ? (
            <div className="dropdown dropdown-end">
              <label tabIndex={0} className="btn btn-xs rounded-full bg-base-100 border border-base-300 text-base-content hover:bg-base-300 font-bold px-2 sm:px-3 flex items-center gap-1 sm:gap-1.5 cursor-pointer normal-case shadow-sm transition-all">
                {user.avatarUrl ? (
                  <img src={user.avatarUrl} alt="Avatar" className="w-5 h-5 rounded-full object-cover border border-base-300 shrink-0" />
                ) : (
                ...
                <span className="hidden sm:block max-w-[100px] truncate text-[11px] font-mono">{user.email}</span>
                <svg className="w-3 h-3 text-base-content/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                  <path d="M19 9l-7 7-7-7" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </label>
              <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[100] p-3 shadow-lg bg-base-200 text-base-content border border-base-300 rounded-2xl w-60 space-y-1">
                <li className="px-3 py-2 border-b border-base-300 mb-1 flex flex-row gap-2.5 items-center">
                  {user.avatarUrl ? (
                    <img src={user.avatarUrl} alt="Avatar" className="w-8 h-8 rounded-full object-cover border border-base-300 shrink-0" />
                  ) : (
                  ...
                  <div className="truncate flex-1">
                    <div className="font-black text-xs truncate p-0 leading-none text-base-content">{user.name}</div>
                    <div className="text-[9px] text-base-content/50 truncate p-0 font-mono mt-1">{user.email}</div>
                  </div>
                </li>
                {user.role === 'learner' && (
                ...
                <li><Link href="/profile">{locale === 'en' ? 'Settings' : 'الإعدادات'}</Link></li>
            ...
            <>
              <Link href="/auth/login" className="hidden sm:inline text-xs font-bold text-base-content/75 hover:text-[#10B981] transition-colors">
                {t('nav.login')}
              </Link>
              <Link
                href="/auth/register"
      ...
      {mobileOpen && (
        <div className="md:hidden mt-2 max-w-5xl mx-auto bg-base-200 border border-base-300 rounded-2xl p-4 space-y-3 shadow-lg text-start">
          ...
          {!user && (
            <Link href="/auth/login" className="block text-sm font-bold text-base-content/75 pt-2 border-t border-base-300">
              {t('nav.login')}
            </Link>
          )}
        </div>
