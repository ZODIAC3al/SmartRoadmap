...
export const ModernTemplate: React.FC<TemplateProps> = ({ cv }) => {
  ...
  return (
    <div className="w-full bg-white text-black p-8 font-sans leading-relaxed select-text space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center md:items-start border-b-2 border-primary pb-6">
        {personal?.photoUrl && (
          <img src={personal.photoUrl} alt="Photo" className="w-20 h-20 rounded-full object-cover border border-gray-200 shadow-sm shrink-0" />
        )}
        <div className="text-center md:text-left flex-grow">
          <h1 className="text-3xl font-bold uppercase tracking-tight text-black">{personal?.name}</h1>
          {personal?.title && <p className="text-sm font-semibold text-primary uppercase tracking-wider mt-1">{personal.title}</p>}
          
          <div className="flex flex-wrap justify-center md:justify-start gap-x-4 gap-y-1 text-xs text-gray-700 mt-3 font-mono">
            ...
            {education.map((edu, idx) => (
              <div key={idx} className="text-xs">
                <div className="flex justify-between items-baseline font-bold text-black">
                  ...
                  <span className="font-mono text-[10px] font-normal text-gray-500">{edu.startDate ? `${edu.startDate} - ` : ''}{edu.graduateDate}</span>
                </div>
          ...
          <div className="space-y-4">
            {workExperience.map((exp, idx) => (
              <div key={idx} className="text-xs">
                <div className="flex justify-between items-baseline font-bold text-black">
                  <span>{exp.role}</span>
                  <span className="font-mono text-[10px] font-normal text-gray-500">{exp.startDate} – {exp.endDate}</span>
                </div>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-1.5">
            {skills.map((skill, idx) => (
              <span key={idx} className="bg-primary/5 text-primary border border-primary/10 text-[11px] font-semibold px-2 py-0.5 rounded">
                {skill}
              </span>
            ))}
          </div>
              ...
              <div key={idx} className="text-xs">
                <div className="flex justify-between items-baseline font-bold text-black">
                  ...
                  {(proj.url || proj.githubUrl) && (
                    <a href={(proj.url || proj.githubUrl || '').startsWith('http') ? (proj.url || proj.githubUrl) : `https://${proj.url || proj.githubUrl}`} target="_blank" rel="noreferrer" className="font-mono text-[10px] font-normal text-primary hover:underline">
                      {proj.url || proj.githubUrl}
                    </a>
                  )}
                </div>
                ...
                {proj.technologies && proj.technologies.length > 0 && (
                  <p className="text-[10px] text-gray-500 mt-1">Tech: {proj.technologies.join(', ')}</p>
                )}
              </div>
            ))}
          </div>
                ...
                <div>
                  ...
                  {c.credentialUrl && (
                    <div className="mt-0.5">
                      <a href={c.credentialUrl.startsWith('http') ? c.credentialUrl : `https://${c.credentialUrl}`} target="_blank" rel="noreferrer" className="text-[10px] text-primary hover:underline font-mono">
                        Verify Credential
                      </a>
                    </div>
                  )}
                </div>
                {c.issueDate && <span className="font-mono text-[10px] text-gray-500">{c.issueDate}</span>}
              </div>
            ))}
          </div>
        </section>
