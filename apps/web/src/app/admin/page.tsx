        ...
        <div className="max-w-md bg-base-200 border border-base-300 p-8 rounded-2xl shadow-sm space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-base-content/50">
              Only master system operations and platform controllers are authorized to access the system metrics index.
            </p>
          </div>
    ...
    <div className="min-h-screen bg-base-100 text-base-content pb-10 px-4 sm:px-6 lg:px-8 text-start font-sans">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-base-200 border border-base-300 rounded-2xl p-6 shadow-sm">
          <div>
            <span className="text-[10px] text-base-content/40 font-mono font-bold uppercase tracking-wider block">Operational Command Console</span>
            <h1 className="text-2xl font-black tracking-tight mt-1">SmartRoadmap Admin Suite</h1>
            <p className="text-xs text-base-content/50 mt-0.5">Control candidate database roles, edit learning modules, and audit system logs.</p>
          </div>
          <div className="flex flex-wrap gap-2.5">
            <Link href="/admin/users" className="btn btn-xs sm:btn-sm btn-outline border-base-300 text-base-content hover:bg-base-100 rounded-xl">
              👥 Manage Users
            </Link>
            <Link href="/admin/content" className="btn btn-xs sm:btn-sm btn-outline border-base-300 text-base-content hover:bg-base-100 rounded-xl">
              📚 Learning Content
            </Link>
            <button 
              onClick={() => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-base-200 border border-base-300 p-5 rounded-2xl shadow-xs">
            <span className="text-[9px] uppercase font-bold text-base-content/40 tracking-wider block font-mono">Total Users</span>
            <span className="text-3xl font-black font-mono text-base-content block mt-1">{stats.totalUsers}</span>
            <span className="text-[9px] text-base-content/40 block font-mono">{stats.totalLearners} Candidates • {stats.totalCompanies} Companies</span>
          </div>

          <div className="bg-base-200 border border-base-300 p-5 rounded-2xl shadow-xs">
            <span className="text-[9px] uppercase font-bold text-base-content/40 tracking-wider block font-mono">Active Roadmaps</span>
            <span className="text-3xl font-black font-mono text-[#059669] block mt-1">{stats.activeRoadmaps}</span>
            <span className="text-[9px] text-base-content/40 block font-mono">Generations successful</span>
          </div>

          <div className="bg-base-200 border border-base-300 p-5 rounded-2xl shadow-xs">
            <span className="text-[9px] uppercase font-bold text-base-content/40 tracking-wider block font-mono">Verified badges</span>
            <span className="text-3xl font-black font-mono text-[#10B981] block mt-1">{stats.quizzesPassed} Passed</span>
            <span className="text-[9px] text-base-content/40 block font-mono">Quiz sessions tracked</span>
          </div>

          <div className="bg-base-200 border border-base-300 p-5 rounded-2xl shadow-xs">
            <span className="text-[9px] uppercase font-bold text-base-content/40 tracking-wider block font-mono">AI Token Costs</span>
            <span className="text-3xl font-black font-mono text-base-content block mt-1">{stats.openaiCosts}</span>
            <span className="text-[9px] text-base-content/40 block font-mono">{stats.tokensUsed} API Tokens</span>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-base-200 border border-base-300 rounded-2xl p-6 shadow-sm space-y-4">
            <div className="flex justify-between items-center border-b border-base-300 pb-3">
              <h3 className="font-extrabold text-xs text-base-content uppercase tracking-wider font-mono">Daily Signup Analytics</h3>
              <span className="text-[9px] bg-[#10B981]/15 text-[#059669] px-2 py-0.5 rounded-full font-bold font-mono">WEEKLY INDEX</span>
            </div>
          ...
          <div className="bg-base-200 border border-base-300 rounded-2xl p-6 shadow-sm space-y-4">
            <div className="flex justify-between items-center border-b border-base-300 pb-3">
              <h3 className="font-extrabold text-xs text-base-content uppercase tracking-wider font-mono">Module Quiz Pass Rates</h3>
              <span className="text-[9px] bg-[#10B981]/15 text-[#059669] px-2 py-0.5 rounded-full font-bold font-mono">BENCHMARK</span>
            </div>
              ...
              {quizPassRates.map((quiz, idx) => (
                <div key={idx} className="space-y-1">
                  <div className="flex justify-between text-[10px] font-semibold text-base-content/70">
                    ...
                    <span className="font-mono text-[#059669] font-bold">{quiz.rate}% Pass</span>
                  </div>
