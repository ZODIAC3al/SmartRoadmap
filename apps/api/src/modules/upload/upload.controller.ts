...
export class UploadController {
  ...
  async uploadImage(@UploadedFile() file: any) {
    ...
    if (!file.mimetype || !file.mimetype.startsWith('image/')) {
      throw new BadRequestException('Invalid file type. Only image files are permitted.');
    }
    ...
    try {
      ...
      throw new BadRequestException(`Image upload pipeline failed: ${error.message}`);
    }
  }
}
