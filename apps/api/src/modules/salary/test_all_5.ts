...
async function runAll5Debug() {
  ...
  console.log('=== SALARY INGESTION PIPELINE DEBUG TEST (ALL 5 MARKETS) ===\n');

  const cases = [
    { country: 'gb', title: 'junior backend developer', label: 'UK' },
    { country: 'us', title: 'junior backend developer', label: 'USA' },
    { country: 'fr', title: 'junior backend developer', label: 'France' },
    { country: 'de', title: 'junior backend developer', label: 'Germany' },
    { country: 'eg', title: 'junior backend developer', label: 'Egypt' },
  ];

  for (const c of cases) {
    console.log(`==================================================`);
    console.log(`[TEST REQUEST] Country: ${c.label} (${c.country.toUpperCase()}) | Job: "${c.title}"`);
