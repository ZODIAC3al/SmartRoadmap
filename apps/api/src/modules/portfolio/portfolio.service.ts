...
export class PortfolioService {
  ...
  async getMyPortfolio(userId: string): Promise<Portfolio> {
    ...
    if (!portfolio) {
      ...
      const username = user?.username || user?.email?.split('@')[0] || `user_${userId.slice(-6)}`;
      ...
      portfolio = new this.portfolioModel({
        ...
        bio: user?.bio || 'Passionate software engineer building web applications.',
        about: user?.bio || '',
        template: 'developer',
        isPublished: false,
        skills: userAny?.skills || ['JavaScript', 'TypeScript', 'React', 'Node.js'],
        socialLinks: {
          email: user?.email,
          github: userAny?.githubUsername ? `https://github.com/${userAny.githubUsername}` : undefined,
  ...
  async savePortfolio(userId: string, data: any): Promise<Portfolio> {
    ...
    let portfolio = await this.getMyPortfolio(userId);
    ...
    if (data.socialLinks !== undefined) portfolio.socialLinks = data.socialLinks;
    ...
    if (data.customSections !== undefined) portfolio.customSections = data.customSections;
    if (data.isPublished !== undefined) portfolio.isPublished = data.isPublished;

    return portfolio.save();
  }

  async setPublishStatus(userId: string, isPublished: boolean): Promise<Portfolio> {
  ...
  async getPublicPortfolio(username: string): Promise<Portfolio> {
    ...
    if (!portfolio) {
      throw new NotFoundException(`Public portfolio for user "${username}" was not found or is private.`);
    }
