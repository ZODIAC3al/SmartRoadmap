...
describe('Remedial Node Generation Trigger', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getModelToken(UserTopicResult.name), useValue: topicResultModel },
        {
    ...
    remedialQueueService = module.get<RemedialNodeQueueService>(RemedialNodeQueueService);
  });
  ...
  it('should queue a BullMQ job when fail_percentage >= REMEDIAL_THRESHOLD', async () => {
    ...
    const result = await service.recordTopicResult('507f191e810c19729de860ea', 'topic_css_layout', false, 40, 'track_frontend');
  ...
  it('should NOT queue a BullMQ job when exam passes (fail_percentage remains low)', async () => {
    ...
    const result = await service.recordTopicResult('507f191e810c19729de860ea', 'topic_html_basics', true, 90, 'track_frontend');
