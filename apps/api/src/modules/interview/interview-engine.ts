...
export class InterviewEngine {
  ...
  async generateFirstQuestion(
    ...
    try {
      ...
      this.logger.warn(
        `LLM question generation failed, using pool: ${e.message}`,
      );
    }
  ...
  async evaluateAnswer(
    ...
    try {
      ...
      const result = await this.llm.complete(evalPrompt, {
        json: true,
        system: 'You are a professional technical interviewer.',
      });
      ...
      if (!answer || answer.trim().length === 0) {
        ...
        idealAnswer =
          'A complete answer would address the key concepts and provide examples.';
        correctness = 'incorrect';
        improvementTips =
          'Try to answer the question even if you are not sure.';
      } else if (answer.trim().length < 30) {
        score = 3;
        feedback =
          'Your answer was very brief. Try to elaborate with examples and details.';
        idealAnswer =
          'A complete answer would address the key concepts and provide examples.';
        correctness = 'partial';
        ...
        score = 6;
        feedback =
          'Good start! You covered the basics. Adding more depth and concrete examples would strengthen this.';
        idealAnswer =
          'Ideally, you would expand on this with real-world scenarios and edge cases.';
        correctness = 'partial';
  ...
  async buildReport(
    ...
    this.logger.log(
      `Starting LLM report generation for session ${sessionId} with ${answers.length} answers.`,
    );
    const total = answers.length || 1;
    const fallbackOverallScore =
      Math.round(
        (answers.reduce((sum, a) => sum + (Number(a.score) || 0), 0) / total) *
          10,
      ) / 10;
    ...
    if (answers.length === 0) {
      this.logger.log(
        `Session ${sessionId} has 0 answers, returning empty fallback report.`,
      );
      return {
        ...baseReport,
        overallScore: 0,
    ...
    const prompt =
      `Analyze this interview transcript for a ${config.difficulty} ${config.type} interview.\n\n${transcript}\n\n` +
    ...
    while (attempts < 3 && !parsed) {
      try {
        attempts++;
        this.logger.log(
          `LLM report request attempt ${attempts} for ${sessionId}`,
        );
        const result = await this.llm.complete(prompt, {
          json: true,
        ...
        this.logger.error(
          `LLM report attempt ${attempts} failed for ${sessionId}: ${err.message}`,
        );
        if (attempts >= 3) break;
      }
    }
    ...
    if (parsed) {
      return {
        ...
        problemSolvingScore:
          Number(parsed.problemSolvingScore) || fallbackOverallScore,
        communicationScore:
          Number(parsed.communicationScore) || fallbackOverallScore,
        ...
        achievements: Array.isArray(parsed.achievements)
          ? parsed.achievements
          : [],
        roadmapUpdates: Array.isArray(parsed.roadmapUpdates)
    ...
    this.logger.warn(
      `Falling back to deterministic report generation for ${sessionId}`,
    );
    ...
    return {
      ...
      communicationScore: Math.max(
        0,
        Math.min(10, fallbackOverallScore + (Math.random() > 0.5 ? 1 : -1)),
      ),
      confidenceScore:
        (answers.filter((a) => a.answer && a.answer.length > 50).length /
          total) *
        10,
      strengths: [...new Set(strengths)],
      weaknesses: [...new Set(weaknesses)],
      recommendations: [
        'Consider practicing more to get a fully AI-generated analysis.',
      ],
