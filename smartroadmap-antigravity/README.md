...
> AI-assisted learning paths, adaptive quiz engines, and talent matching platform — Next.js 14 & NestJS 11 monorepo utilizing multi-provider LLM orchestration (Google Gemini, Groq, OpenAI), adaptive assessments, AWS-style verifiable shield badges, and full-page high-resolution certification export.

[![CI/CD Pipeline](https://img.shields.io/badge/CI%2FCD-PASSED%20100%25-emerald?style=for-the-badge&logo=github-actions)](https://github.com/Devotopia/SmartRoadmap)
[![TypeScript](https://img.shields.io/badge/TypeScript-0%20Errors-blue?style=for-the-badge&logo=typescript)](https://typescriptlang.org)
[![NestJS Unit Tests](https://img.shields.io/badge/NestJS%20Tests-20%2F20%20PASSED-brightgreen?style=for-the-badge&logo=nestjs)](https://nestjs.com)
[![Next.js 14](https://img.shields.io/badge/Next.js-14%20App%20Router-black?style=for-the-badge&logo=next.js)](https://nextjs.org)
[![Bilingual AR/EN](https://img.shields.io/badge/i18n-Arabic%20%26%20English-purple?style=for-the-badge)](https://smartroadmap.io)
...
- **AI Curriculum Designer:** Based on a desired target role and current skills, the system uses multi-provider LLM orchestration (Gemini 2.5 Flash, Groq Llama 3.3 70B) to generate a custom visual learning roadmap composed of modules with estimates and visual tree coordinates.
- **Adaptive Skill Assessment:** Each learning module features an adaptive quiz. The questions rise or lower in difficulty (`easy` -> `medium` -> `hard`) based on consecutive correct or incorrect answers. Scores are weighted by question difficulty (`easy=1.0`, `medium=1.5`, `hard=2.0`).
...
- **Verifiable Shield Badges System:** 10 custom AWS-style shield badges (*AI Architect*, *Solutions Architect*, *Cloud Practitioner*, *Security Engineer*, *DevOps Pro*, *Fullstack Master*, *Data Engineer*, *ML Specialist*, *DB Architect*, *Cyber Defense*) stored in MongoDB with standalone SVG vector export support.
- **Full-Page High-Resolution Certificate Export:** Issue official track credential passports upon passing all track modules. Export printable A4 landscape certificates with zero margins and high-definition CSS `@media print` rules.
- **Interactive Analytics & Progress Charts:** Real-time study history trackers, daily activity analytics (7d / 30d / 90d), radial pass-rate bar charts, and quiz score performance line graphs built with Recharts.
- **Bilingual AR / EN Responsiveness:** Full Arabic and English language support across all dashboards, navbars, modals, quizzes, and speech notes, with automatic RTL / LTR layout adjustments.
...
- **Premium Subscriptions:** Access is gated by plan levels (`free`, `pro_learner`, `company_tier`) using a secure transaction flow with server-side price verification.
