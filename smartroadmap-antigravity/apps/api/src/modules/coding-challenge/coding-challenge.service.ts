...
export class CodingChallengeService implements OnModuleInit {
    ...
    const challenges = [
      {
        ...
        prompt:
          'Write a function `fizzBuzz(n)` returning an array of strings from 1 to n. For multiples of 3 return "Fizz", for 5 return "Buzz", for both return "FizzBuzz".',
      ...
      },
      {
        ...
        prompt:
          'Write a function `reverseString(str)` that takes a string input and returns its reverse character order.',
        difficulty: 'easy' as const,
  ...
  async getChallenges(moduleId?: string, userId?: string) {
    ...
    const attemptsMap = new Map(
      attempts.map((a) => [a.challengeId.toString(), a.passed]),
    );
  ...
  async submitCode(
    ...
    for (const tc of challenge.testCases) {
      ...
      const wrappedCode = this.wrapCodeForTest(
        language,
        code,
        challenge.title,
        tc.input,
      ...
      const execution = await this.executionService.runCode(
        language,
        wrappedCode,
        tc.input,
      );
      ...
      const cleanActual = execution.stdout
        .trim()
        .replace(/^['"`]|['"`]$/g, '')
        .replace(/\s+/g, '');
      const cleanExpected = tc.expectedOutput
        .trim()
        .replace(/^['"`]|['"`]$/g, '')
        .replace(/\s+/g, '');
    ...
    if (allPassed) {
      this.logger.log(
        `User ${userId} passed coding challenge ${challengeId}! Emitting event.`,
      );
