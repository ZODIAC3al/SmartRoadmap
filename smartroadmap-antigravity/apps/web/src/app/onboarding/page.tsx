...
const PRESET_ROLES = [
  {
    ...
    desc: "Build reactive user interfaces, state managers, and modern layouts.",
    icon: "⚛️",
  },
  {
    id: "be",
    title: "Backend Developer",
    desc: "Design microservices, API architectures, databases, and message queues.",
    icon: "⚙️",
  },
  {
    id: "ds",
    title: "Data Scientist",
    desc: "Process datasets, design machine learning algorithms, and build models.",
    icon: "📊",
  },
  {
    id: "do",
    title: "DevOps Engineer",
    desc: "Deploy cloud infrastructure, orchestrate containers, and automate CI/CD.",
    icon: "🚀",
  },
];
...
export default function OnboardingPage() {
  ...
  const loadingMessages = [
    locale === "en"
      ? "Analyzing your background diagnostics..."
      : "جاري تحليل تشخيصات خلفيتك الفنية...",
    locale === "en"
      ? "Mapping syllabus dependency graphs..."
      : "جاري رسم خرائط تبعية المناهج الدراسية...",
    locale === "en"
      ? "Optimizing modules sequence parameters..."
      : "جاري تحسين تسلسل الوحدات والدروس...",
    locale === "en"
      ? "Compiling personalized learning roadmap..."
      : "جاري تجميع خارطة الطريق التعليمية المخصصة...",
  ];
  ...
  const handleNext = () => {
    if (step === 1 && !targetRole && !customRoleInput) {
      alert(
        locale === "en"
          ? "Please select a role or input a custom goal."
          : "يرجى اختيار مسمى وظيفي أو كتابة هدف مخصص.",
      );
      return;
    }
    setStep((prev) => prev + 1);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(
        locale === "en"
          ? "Failed to generate your roadmap. Please check if your backend server is active."
          : "فشل إنشاء خارطة الطريق. يرجى التحقق من تشغيل الخادم الخلفي.",
      );
      setStep(3);
    }
  };
  ...
  const getRoleTitle = (roleId: string, def: string) => {
    if (locale === "ar") {
      switch (roleId) {
        case "fe":
          return "مهندس واجهات أمامية";
        case "be":
          return "مطور برمجيات خلفية";
        case "ds":
          return "عالم بيانات";
        case "do":
          return "مهندس عمليات سحابية (DevOps)";
        default:
          return def;
      }
    }
  ...
  const getRoleDesc = (roleId: string, def: string) => {
    if (locale === "ar") {
      switch (roleId) {
        case "fe":
          return "بناء واجهات تفاعلية للمستخدم، وإدارة الحالة، والتصميمات الحديثة.";
        case "be":
          return "تصميم بنية الخدمات المصغرة، وقواعد البيانات، وأنظمة الرسائل البرمجية.";
        case "ds":
          return "معالجة مجموعات البيانات الكبيرة، وتصميم لوغاريتمات ونماذج التعلم الآلي.";
        case "do":
          return "تهيئة البنية التحتية السحابية، وإدارة الحاويات وأتمتة خطوط النشر.";
        default:
          return def;
      }
    }
  ...
  const getEduLabel = (edu: string) => {
    if (locale === "ar") {
      switch (edu) {
        case "Bootcamp/Self-Taught":
          return "معسكر تدريبي / تعليم ذاتي";
        case "College Degree":
          return "شهادة جامعية";
        case "High School Graduate":
          return "شهادة الثانوية العامة";
        case "Post-Graduate":
          return "دراسات عليا / ماجستير / دكتوراه";
        default:
          return edu;
      }
    }
        ...
        <div className="card-body">
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-6">
                {PRESET_ROLES.map((role) => (
                  <div
                    ...
                    className={`card bg-base-100 border p-4 cursor-pointer hover:border-primary transition-all ${
                      targetRole === role.title && !isCustomRole
          ...
          {step === 2 && (
            <div>
              ...
              <div className="form-control mb-6">
                <label className="label font-bold text-xs sm:text-sm">
                  {t("onboard.edu_label")}
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {EDUCATION_OPTIONS.map((edu) => (
                    <label
                      key={edu}
                      className={`card bg-base-100 border p-4 cursor-pointer hover:border-primary flex flex-row gap-3 items-center ${
                        education === edu
                          ? "border-primary border-2 shadow-sm"
                          : "border-base-300"
                      }`}
              ...
              <div className="card-actions justify-between">
                <button
                  onClick={handleBack}
                  className="btn btn-outline btn-ghost text-xs sm:text-sm"
                >
                  {locale === "en" ? "← Back" : "رجوع →"}
                </button>
                <button
            ...
            <div className="py-12 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-xl sm:text-2xl font-bold mb-2 text-base-content">
                {locale === "en"
                  ...
                  : "جاري بناء خارطة طريقك التفاعلية"}
              </h3>
